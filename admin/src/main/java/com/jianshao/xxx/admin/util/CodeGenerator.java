package com.jianshao.xxx.admin.util;
...
public class CodeGenerator {
	 ...
	 * 数据库连接url
	 */
	private static final String DB_URL = "jdbc:mysql://localhost:3306/appointment_service?serverTimezone=GMT%2B8&characterEncoding=utf-8";
	 ...
	 * 密码
	 */
	private static final String PASSWORD = "C=w9nM3_X";
	 ...
	 */
	private static final String PACKAGE_NAME = "com.jianshao.appointment_service.admin";
	...
	private static void generateByTables(String tableNames){
		...
		focList.add(new FileOutConfig("/templates/entityReq.java.vm") {
			@Override
			public String outputFile(TableInfo tableInfo) {
				return BASE_PATH + PACKAGE_NAME.replaceAll("\\.","/") + "/" + tableInfo.getEntityPath().toLowerCase() +"/controller/"+ tableInfo.getEntityName() + "Req" + StringPool.DOT_JAVA;
			}
		});
	...
	protected List<TableInfo> getAllTableInfoList(ConfigBuilder config) {
		...
		allTableInfoList.forEach(tableInfo -> {
			...
			fields.forEach(field -> {
				...
				if ("LocalDate".equals(field.getPropertyType())) {
					field.setColumnType(DbColumnType.DATE);
